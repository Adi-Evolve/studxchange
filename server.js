...
// Middleware
app.use(cors());
app.use(bodyParser.json({ limit: '100mb' })); // Reduced limit for better performance
app.use(bodyParser.urlencoded({ extended: true, limit: '100mb' })); // Reduced limit for better performance
app.use(express.json({ limit: '100mb' })); // Added limit to express.json
...
app.post('/api/upload-pdf', (req, res) => {
  try {
    // Just acknowledge the upload (actual file is already on Google Drive)
    res.json({ success: true });
  } catch (error) {
    console.error('PDF upload notification error:', error);
    res.status(500).json({ message: 'Failed to process upload notification' });
  }
});
